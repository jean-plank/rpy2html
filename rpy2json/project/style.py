...
def new_style(gui, config, fonts_usages, Borders):
    ...
    res['textbox_yalign'] = yalign(
        gui.textbox_yalign, gui.textbox_height, HEIGHT)
    ...
    res['namebox_width'] = get_or_else(
        percent(gui.namebox_width, WIDTH), 'auto')
    res['namebox_height'] = get_or_else(
        percent(gui.namebox_height, gui.textbox_height), 'auto')
    res['namebox_left'] = percent(gui.name_xpos, WIDTH)
    res['namebox_top'] = get_or_else(
        percent(gui.name_ypos + 5, gui.textbox_height), percent(5, gui.textbox_height))
    ...
    res['guibtn_padding'] = padding(
        guibtn_borders(gui, Borders), mmenuitems_width)
    ...
    res['guibtn_width'] = get_or_else(
        percent(gui.button_width, mmenuitems_width), 'auto')
    res['guibtn_height'] = get_or_else(
        percent(gui.button_height, HEIGHT), 'auto')
    ...
    res['choice_yalign'] = yalign(
        1.0 - gui.textbox_yalign, HEIGHT - gui.textbox_height, HEIGHT)
    ...
    res['choicebtn_width'] = get_or_else(
        percent(gui.choice_button_width, WIDTH), 'auto')

    # save slots
    slot_fsize = fontsize(HEIGHT, WIDTH, gui.slot_button_text_size)
    res['slot_width'] = percent(
        gui.slot_button_width, WIDTH - mmenuitems_width)
    res['slot_height'] = percent(gui.slot_button_height, HEIGHT)
    res['slot_padding'] = padding(
        gui.slot_button_borders, WIDTH - mmenuitems_width)
    ...
    res['thumb_height'] = percent(
        config.thumbnail_height, gui.slot_button_height)
    res['thumb_margin_top'] = percent(
        gui.slot_button_borders.top * 2, HEIGHT - config.thumbnail_height)
    ...
    # title
    title_fsize = fontsize(HEIGHT, WIDTH, gui.title_text_size)
    res['title_fsize_v'] = title_fsize['v']
    res['title_fsize_h'] = title_fsize['h']
...
def old_style(gui, config, fonts_usages, Borders):
    ...
    res['choice_yalign'] = yalign(
        1.0 - textbox_yalign, HEIGHT - textbox_height, HEIGHT)
    ...
    res['slot_padding'] = padding(
        Borders(15, 15, 15, 15), WIDTH - mmenuitems_width)
    res['slot_fsize_v'] = slot_fsize['v']
    res['slot_fsize_h'] = slot_fsize['h']
    ...
    # title
    title_fsize = fontsize(HEIGHT, WIDTH, 75)
    res['title_fsize_v'] = title_fsize['v']
    res['title_fsize_h'] = title_fsize['h']
